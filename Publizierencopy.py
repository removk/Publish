...
class Scheduler:
    ...
    def scheduleExecution(self, task, selected_date=None, repeat_weekly=False):
        ...
        if delay <= 0:
            messagebox.showerror("Fehler", "Das ausgewählte Datum und die Uhrzeit müssen in der Zukunft liegen.")
            return

        self.timer = Timer(delay, self._executeTask, [task, repeat_weekly, selected_date])
        self.timer.start()
        messagebox.showinfo("Erfolg", f"Task wird für {selected_date} geplant.")
